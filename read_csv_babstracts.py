...
import csv
import uuid
# This is a special version I created to read the abstracts that Bill Duncan gave us
# They have the unusual feature that 1 or 0 in some columns indicates a specific isAbout prop value
...
ceramic_concept = conn.createURI("http://www.semanticweb.org/ontologies/2022/titutuli/nivedita/drmo#Ceramic")
resin_based_concept = conn.createURI("http://www.semanticweb.org/ontologies/2022/titutuli/nivedita/drmo#ResinBasedComposite")
...
def find_property(prop_str):
    ...
    prop = conn.createURI(str(prop_str))
    print(prop)
            ...
            if line_count == 0:
                ...
                while i < row_count:
                    ...
                    if i < 6:
                        ...
                        nextval = row[i]
                        if nextval == str(1):
                            conn.add(new_csr_iri, proplist[i], ceramic_concept)
                        i += 1
                    else:
                        nextval = row[i]
                        if nextval == str(1):
                            conn.add(new_csr_iri, proplist[i], resin_based_concept)
                        i += 1
                line_count += 1
...
# print(find_property("http://purl.org/dc/terms/identifier"))
